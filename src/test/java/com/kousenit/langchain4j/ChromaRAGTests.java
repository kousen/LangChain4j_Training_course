 ...
 *
 * <p>This lab demonstrates how to use Chroma as a persistent vector store for production RAG
 * systems. You'll learn how to: - Set up Chroma as a vector database for embeddings - Create
 * persistent RAG systems with Chroma - Implement production-ready RAG configurations - Manage
 * vector data with Chroma - Scale RAG applications with persistent storage
 *
 * <p>Prerequisites: - Docker with Chroma running: docker run -p 8000:8000 chromadb/chroma:0.5.4 -
 ...
 *
 * <p>IMPORTANT: Use Chroma version 0.5.4 for compatibility with LangChain4j 1.0.1
 *
...
class ChromaRAGTests {
   ...
   *
   * <p>Demonstrates fundamental vector store capabilities: - Creating a Chroma embedding store -
   * Storing and searching document embeddings - Basic vector similarity search - Data persistence
   * verification
   */
  @Test
  void chromaVectorStoreOperations() {
    ...
    //     Document.from("Python is a high-level programming language known for its simplicity and
    // readability."),
    //     Document.from("Java is a popular object-oriented programming language that runs on the
    // JVM."),
    //     Document.from("JavaScript is the language of the web, used for both frontend and backend
    // development."),
    //     Document.from("Rust is a systems programming language focused on safety and
    // performance."),
    //     Document.from("Go is a statically typed, compiled programming language designed for
    // building scalable
    // systems.")
    // );
   ...
   *
   * <p>Demonstrates a complete production-ready RAG implementation: - Comprehensive knowledge base
   * with metadata - Optimized retrieval configuration - AI-powered question answering - Production
   * model settings and error handling
   */
  @Test
  void productionRagSystem() {
    ...
    // List<Document> documents = List.of(
    //     Document.from("LangChain4j 1.0 introduced the ChatModel interface as the primary way to
    // interact with
    // language models."),
    //     Document.from("The AiServices interface in LangChain4j allows you to create type-safe
    // AI-powered services
    // using annotations."),
    //     Document.from("LangChain4j supports multiple embedding models including OpenAI embeddings
    // and local
    // models like AllMiniLM."),
    //     Document.from("ContentRetriever in LangChain4j is used to retrieve relevant content for
    // RAG
    // applications."),
    //     Document.from("LangChain4j provides built-in support for Chroma as a vector store for
    // production RAG
    // systems."),
    //     Document.from("The @Tool annotation enables AI models to call Java methods during
    // conversations."),
    //     Document.from("RAG (Retrieval-Augmented Generation) allows AI to access external
    // knowledge sources for
    // better answers."),
    //     Document.from("LangChain4j uses builder patterns throughout the library for configuring
    // AI services and
    // models.")
    // );
   ...
   *
   * <p>Demonstrates realistic document processing with file loading and parsing: - Loading
   * documents from the filesystem - Parsing PDF documents using Apache Tika - Processing real
   * document content through the RAG pipeline - Querying parsed document content with AI
   *
   * <p>Note: This test uses a real PDF file containing LangChain4j 1.0+ documentation,
  ...
  void ragWithDocumentParsing() {
    ...
    //     @SystemMessage("You are an expert assistant that answers questions based on technical
    // documentation. " +
    //                   "Provide accurate, detailed answers based on the provided context from the
    // document. " +
    //                   "If the document doesn't contain enough information to fully answer the
    // question, " +
  ...
  /**
   * Helper method to check if Chroma is available. Attempts to connect to Chroma on localhost:8000.
   */
