 ...
 * You'll learn how to: - Connect to external MCP servers using LangChain4j MCP client - Use the
 * official MCP "everything" demo server - Integrate external MCP tools with LangChain4j AiServices
 ...
 * running the MCP "everything" server - MCP "everything" server accessed via: npx
 * -y @modelcontextprotocol/server-everything
 *
 * <p>Key Concepts: - MCP allows AI applications to access tools from external services -
 * LangChain4j provides MCP client support (not server) - External MCP tools integrate seamlessly
 * with local @Tool methods - MCP enables distributed tool ecosystems
 */
class McpIntegrationTests {
   ...
   *
   * <p>Steps to implement: 1. Create StdioMcpTransport with Docker command for "everything" server
   * 2. Create DefaultMcpClient with unique key and the transport 3. Create McpToolProvider using
   * the MCP client 4. Verify both client and tool provider are created successfully 5. No need for
   * server availability checks - Docker handles this
   */
  @Test
   ...
   *
   * <p>Steps to implement: 1. Configure ChatModel with OpenAI GPT-4-1-Nano 2. Create MCP transport
   * and client using stdio approach 3. Create McpToolProvider from the MCP client 4. Define AI
   * assistant interface 5. Build AI service with .toolProvider(mcpToolProvider) 6. Test asking
   * about available tools and verify responses
   */
  @Test
   ...
   * tools together.
   *
   * <p>Steps to implement: 1. Configure ChatModel 2. Create MCP client and tool provider using
   * stdio approach 3. Build AI service with both .tools() (DateTimeTool only - avoid CalculatorTool
   * conflicts) and .toolProvider() 4. Test questions that require both local and external tools 5.
  ...
  void combiningLocalAndMcpTools() {
    ...
    // // Note: Using only DateTimeTool to avoid conflicts with MCP server tools (e.g., "add"
    // function)
    // HybridAssistant assistant = AiServices.builder(HybridAssistant.class)
    //         .chatModel(chatModel)
    //         .tools(new DateTimeTool()) // Local tools - avoiding CalculatorTool due to potential
    // conflicts
    //         .toolProvider(mcpToolProvider) // External MCP tools
    //         .build();

    // TODO: Test combining local and external tools
    // String response1 = assistant.chat("What's the current date and time, and what tools do you
    // have available?");
    // String response2 = assistant.chat("What's the current date, and can you also tell me what MCP
    // tools you can
    // access?");
   ...
   *
   * <p>Steps to implement: 1. Configure ChatModel 2. Create MCP client using stdio approach 3.
   * Create McpToolProvider with configuration options 4. Build AI service with the tool provider 5.
   * Test the configured tool provider functionality
   */
  @Test
