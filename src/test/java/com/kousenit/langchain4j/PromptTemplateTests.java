 ...
 *
 * <p>This lab demonstrates how to use prompt templates with LangChain4j. You'll learn how to: -
 * Create and use simple prompt templates with variable substitution - Load templates from resource
 * files - Use templates with AiServices annotations - Work with different data types in template
 * variables
 */
class PromptTemplateTests {
   ...
   *
   * <p>TODO: Implement a basic prompt template with variable substitution 1. Create an OpenAI chat
   * model 2. Create a PromptTemplate with {{variable}} placeholders 3. Create a Map with variable
   * values 4. Apply the template to generate a Prompt 5. Send the prompt to the model and verify
   * the response
   */
  @Test
  void simplePromptTemplate() {
    ...
    // PromptTemplate template = PromptTemplate.from("Tell me {{count}} movies whose soundtrack was
    // composed by
    // {{composer}}");
   ...
   *
   * <p>TODO: Implement template loading from a resource file 1. Create a template file at
   * src/main/resources/movie_prompt.mustache 2. Load the template content from the resource 3.
   * Create a PromptTemplate from the loaded content 4. Apply variables and generate response 5.
   * Verify the response
   */
  @Test
  void promptTemplateFromResource() throws IOException {
    ...
    // assertNotNull(response);
    // assertTrue(response.toLowerCase().contains("michael") ||
    // response.toLowerCase().contains("giacchino"));
  }
  ...
  interface MovieService {
    ...
    String getMoviesByGenreAndDecade(
        @V("count") int count, @V("genre") String genre, @V("decade") String decade);
  }

  /**
   * Test 4.3: Advanced Template with AiServices
   *
   * <p>TODO: Implement template usage with AiServices annotations 1. Create an OpenAI chat model 2.
   * Build a MovieService using AiServices 3. Call the service method with template variables 4.
   * Verify the response contains expected content 5. Test multiple template methods
   */
  @Test
  void templateWithAiServices() {
    ...
    // assertTrue(hanZimmerMovies.toLowerCase().contains("hans") ||
    // hanZimmerMovies.toLowerCase().contains("zimmer"));
  ...
  interface RestaurantService {
    ...
    @dev.langchain4j.service.UserMessage(
        "Recommend {{count}} {{cuisine}} restaurants in {{city}} with a budget of ${{budget}} per person")
    ...
    @dev.langchain4j.service.UserMessage(
        "Create a {{mealType}} menu for {{people}} people with dietary restrictions: {{restrictions}}")
    String createMenu(
        @V("mealType") String mealType,
        @V("people") int people,
        @V("restrictions") String restrictions);
  }

  /**
   * Test 4.4: Complex Template Variables
   *
   * <p>TODO: Implement templates with multiple variable types and system messages 1. Create an
   * OpenAI chat model 2. Build RestaurantService using AiServices 3. Test templates with different
   * data types (int, double, String) 4. Test system message combined with user message templates 5.
  ...
  void complexTemplateVariables() {
    ...
    // String recommendations = service.getRestaurantRecommendations(3, "Italian", "San Francisco",
    // 75.50);
    // System.out.println("Restaurant Recommendations: " + recommendations);
    // assertNotNull(recommendations);
    // assertTrue(recommendations.toLowerCase().contains("italian") ||
    // recommendations.toLowerCase().contains("san
    // francisco"));
    ...
    // assertNotNull(menu);
    // assertTrue(menu.toLowerCase().contains("vegetarian") ||
    // menu.toLowerCase().contains("gluten"));
  }
}
