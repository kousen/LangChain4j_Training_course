 ...
 *
 * <p>When the LLM omits an argument, LangChain4j substitutes the declared default before invoking
 * the method. The parameter is also marked optional in the JSON schema, so the model is explicitly
 * told it may skip it. Compare to {@code Optional<T>} (Lab 6.5): pick {@code Optional} when absence
 * is meaningful business logic, and {@code defaultValue} when the tool has a sensible fallback the
 * LLM shouldn't have to think about.
 *
 * <p>Used in Lab 6: AI Tools exercises.
 */
